...
public class WebScreenshotUtils {
    ...
    /**
     * 每个线程拥有独立的WebDriver实例，解决并发访问问题
     */
    private static final ThreadLocal<WebDriver> webDriverThreadLocal =
            ThreadLocal.withInitial(WebScreenshotUtils::createWebDriver);

    /**
     * 保存所有已创建的WebDriver实例，用于JVM关闭时统一清理
     */
    private static final List<WebDriver> allDrivers = new CopyOnWriteArrayList<>();

    static {
        // 注册JVM关闭钩子，确保所有WebDriver实例被正确关闭
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            log.info("JVM关闭中，开始清理所有WebDriver实例...");
            allDrivers.forEach(driver -> {
                try {
                    driver.quit();
                } catch (Exception e) {
                    log.warn("关闭WebDriver实例时发生异常", e);
                }
            });
            log.info("所有WebDriver实例已清理完成");
        }));
    }
    ...
    public static String saveWebPageScreenshot(String webUrl) {
        ...
        try {
            ...
            String rootPath = System.getProperty("user.dir") + File.separator + "tmp" + File.separator + "screenshots"
                    + File.separator + RandomUtil.randomString(8);
    ...
    private static WebDriver createWebDriver() {
        try {
            ...
            ChromeOptions options = getChromeOptions(DEFAULT_WIDTH, DEFAULT_HEIGHT);
            WebDriver driver = new ChromeDriver(options);
            driver.manage().timeouts().pageLoadTimeout(Duration.ofSeconds(30));
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
            // 记录到全局列表，以便JVM关闭时清理
            allDrivers.add(driver);
            log.info("创建新的WebDriver实例，当前活跃实例数: {}", allDrivers.size());
            return driver;
        } catch (Exception e) {
